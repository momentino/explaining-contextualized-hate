...
def predict_proba(input, model, tokenizer, device):
    ...
    input = list(input) if isinstance(input,
                                      np.ndarray) else input  # convert to list if it is numpy array else leave as it is
    model.eval()
    with torch.no_grad():
...
def eval_explanations(original_texts, no_rationales, only_rationales, model, tokenizer, device):
    ...
    for original_text, text_no_rationales, text_only_rationales in zip(original_texts, no_rationales, only_rationales):
        ...
        no_rationales_proba = predict_proba(text_no_rationales, model, tokenizer,
                                            device) if text_no_rationales != "" else [[0, 0, 0]]
        only_rationales_proba = predict_proba(text_only_rationales, model, tokenizer,
                                              device) if text_only_rationales != "" else [[0, 0, 0]]
