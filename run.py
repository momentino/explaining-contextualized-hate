...
def main(args):
    ...
    if not (filtered_df['f1'] > test_f1).any():
        ...
        dataset_to_jsonl(dataset_df, 'train', os.path.join('datasets/'+dataset_name+'/data/'+context_path+'/best_run_splits','train.jsonl'))
        dataset_to_jsonl(dataset_df, 'val', os.path.join('datasets/'+dataset_name+'/data/'+context_path+'/best_run_splits','val.jsonl'))
        dataset_to_jsonl(dataset_df, 'test', os.path.join('datasets/'+dataset_name+'/data/'+context_path+'/best_run_splits','test.jsonl'))
    """ Save results """
